...
while step<=20000: #step<=1000:
    ...
    tmp = time.time() - start1
    
    total_time1 = total_time1 + tmp    
    
    states= states['rgb']
    
    start2 = time.time()
    
    states = preprocess_image_rgb(states)  #I think this is slow
    
    tmp = time.time() - start2
    
    total_time2 = total_time2 + tmp    
    ...
    total_time3 = total_time3 + tmp 
    
    actions, action_log_probs, state_value_preds, entropy = select_action( states, actor, critic )    #I think this is slow   #np.array([9]).reshape(-1)
    ...
    tmp = time.time() - start4
    
    total_time4 = total_time4 + tmp    
