# -*- coding: utf-8 -*-
"""
Created on Sat Aug 10 17:54:23 2024
...
# -*- coding: utf-8 -*-
"""
Created on Sat Aug 10 10:02:16 2024
...
def main():
    ...
    try:
        ...
        def select_action(x, actor, critic):
            ...
            with autocast():
                start = time.time()
                #action_pd = torch.softmax(action_logits, dim=-1) 
                ...
                start = time.time()
                #actions = torch.multinomial(action_pd, num_samples=1) 
        ...
        num_envs = 20 #50 #was 20 #10 #was 20 #worked with one or 2 envs till now
        ...
        total_reward = 0
        #row = pd.read_csv("C:/Users/gauthambekal93/Research/rl_generalization_exps/rl_procgen_games/Experiment_V2/Results.csv")
        
        #if len(row)<=1: 
        #    time_step = 0
        #else:
        #    time_step = row.iloc[-1]['Time_Steps']
        
        time_step =0
        
        logging_rate = 10000   #A level can take at most be of 1000 time steps, so we took it as same. But there is no specific reason to take 1000.
        #train_rewards = {}
        model_load = False #False
        ...
        with open("Results.csv",'a',newline='') as file:
            ...
            writer.writerow([ "Train", logging_rate , total_reward, total_reward / logging_rate ]  )  
        ...
        while step<=20000: #step<=1000:
            
            print("Done ",done,"step ", step)    
            
            action, action_log_probs, state_value_preds, entropy = select_action( state, actor, critic )  #np.array([9]).reshape(-1)
            
            #action, action_pd= select_action( state, actor, critic )  #np.array([9]).reshape(-1)
            
            #start = time.time()
            #action = np.random.randint(0, 15 , size=1)   
            action = np.array( action.detach().cpu())[0]
            #tmp = time.time() - start
            #total_duration2 = total_duration2 + tmp
            #print("Tmp2 ", tmp)
            
            
            #start = time.time()
            new_state, reward, done, info = env.step(action) #env.step(action[0])
            #tmp = time.time() - start
            #print("Tmp3 ", tmp)
            #total_duration3 = total_duration3 + tmp
            
            #start = time.time()
            new_state = np.expand_dims(new_state, axis=0)
            #tmp = time.time() - start
            #print("Tmp4 ", tmp)
            #total_duration4 = total_duration4 + tmp
    
            #start = time.time()
            new_state = preprocess_image_rgb(new_state)
            #tmp = time.time() - start
            #print("Tmp5 ", tmp)
            #total_duration5 = total_duration5 + tmp
          
            #start = time.time()
            state, frames = stack_frames(frames, new_state, num_envs, is_new_episode=False)
            #tmp = time.time() - start
            #total_duration6 = total_duration6 + tmp
            #print("Tmp6 ", tmp)
            
            step +=1
            
        print("Total time steps ", time_step,"Time take ", time.time()- start )    
        print("stop")
        print("stop")
