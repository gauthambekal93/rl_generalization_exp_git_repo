...
def test_model(actor, critic, time_elasped, envs_test, result_path, logging_rate ):
    ...
    while time_step <= logging_rate: #total_test_duration:
    ...
    current_reward_rate = total_reward / logging_rate
    
    print(  "Test: ", "Time Step: ", str(time_elasped) , "Reward: ", str(total_reward), "Reward Rate: ",str(current_reward_rate) )
    
    with open(os.path.join(result_path, "Results.csv") ,'a',newline='') as file:
        writer = csv.writer(file)
        writer.writerows( [[ "Test", str(time_elasped) , str(total_reward), str(current_reward_rate) ]]  )  
        file.close()
